...
import time #on importe le module time ce qui permettra d'utiliser des fonction comme time.sleep()
...
GPIO.setup(7, GPIO.IN) #on initialise le port utilisé dans ce cas si c'est le 7 pour le capteur. Il y est rentré la valeur in car le capteur envoie vers le micro processeur c'est une entrée.
GPIO.setup(17, GPIO.OUT)#ici on fait pareil, ce port serivra au moteur pour un send et il est out car les info viennent du microprocesseur vers le moteur
...
time.sleep(2) #la fonction sleep() permet de creer un delay avant la suite du code soit 2 secondes ici
   ...
   time.sleep(0.1)
    ...
    while True:
        try:
            ...
            GPIO.output(18, True)
            time.sleep(2)
        except(KeyboardInterrupt): #ici cette commande permet de faire une execption au prgramme seulement si "controle-c" est pressé
